...
import { anxietyQuizQuestions, getTotalAnxietyQuestions } from "@/data/anxietyQuizData";
import { ArrowLeft, ArrowRight, CheckCircle } from "lucide-react";

export default function AnxietyQuestionPage() {
	...
	useEffect(() => {
		const storedAnswers = localStorage.getItem("anxietyQuizAnswers");
		if (storedAnswers) {
	...
	const currentQuestion = anxietyQuizQuestions.find((q) => q.id === questionId);
	...
	const handleNext = () => {
		...
		localStorage.setItem("anxietyQuizAnswers", JSON.stringify(updatedAnswers));
		...
		if (isLastQuestion) {
			...
			router.push(`/anxiety/results/${resultId}`);
		} else {
			router.push(`/anxiety/question/${questionId + 1}`);
		}
	};
...
function calculateResultId(answers: Record<number, number>): string {
	...
	Object.entries(answers).forEach(([questionId, optionIndex]) => {
		const question = anxietyQuizQuestions.find((q) => q.id === parseInt(questionId));
		if (question && question.options[optionIndex]) {
			const option = question.options[optionIndex];
			scores.generalized += option.scores.generalized;
			scores.social += option.scores.social;
			scores.panic += option.scores.panic;
		}
	});

	// Store scores in localStorage for results page
	localStorage.setItem("anxietyQuizScores", JSON.stringify(scores));

	// Generate a simple result ID
